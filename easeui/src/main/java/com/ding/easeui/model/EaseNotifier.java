...
public class EaseNotifier {
    private final static String TAG = "notify_Debugs";
    ...
    public synchronized void onNewMsg(EMMessage message) {
        ...
        if (PreferenceManager.getInstance().getGroupMsgNobibi(message.getTo()).equals("true")) {
            return;
        }
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        Log.e(TAG, "realnamefromextra=" + realnamefromextra);
        //将id换位真实姓名。写个判断谁有他的对应的名字就用谁的
        if (realnamefromextra != null) {
            username = realnamefromextra;
        } else {
            if (mCache.getAsString(username + "_realname") != null) {
                username = mCache.getAsString(username + "_realname");
            }
        }
        try {
