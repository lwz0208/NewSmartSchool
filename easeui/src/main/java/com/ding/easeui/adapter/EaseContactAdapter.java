...
public class EaseContactAdapter extends ArrayAdapter<EaseUser> implements
        SectionIndexer {
    ...
    public EaseContactAdapter(Context context, int resource,
                              List<EaseUser> objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            if (res == 0)
                convertView = layoutInflater.inflate(R.layout.ease_row_contact,
                        null);
            else
        ...
        boolean isChinese = CommonUtils.isChinesewords(holder.nameView.getText().toString());
//        boolean isNum = holder.nameView.getText().toString().matches("[0-9]+");
        String realname = mCache.getAsString(username + "_realname");
        Log.e("isNum", isChinese + "");
        if (!isChinese) {
            if (realname == null || realname.equals("")) {
                try {
                    CommonUtils.getNickname(mCt, username, holder.nameView);
                ...
                holder.nameView.setText(realname);
            }

        }
        // 设置头像，弃用环信的方法，采用自己的方法显示头像
