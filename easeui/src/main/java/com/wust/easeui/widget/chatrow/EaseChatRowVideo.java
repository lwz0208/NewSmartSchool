...
import com.wust.easeui.model.EaseImageCache;
...
import com.wust.easeui.ui.EaseShowVideoActivity;
...
public class EaseChatRowVideo extends EaseChatRowFile
{
	...
	public EaseChatRowVideo(Context context, EMMessage message, int position,
			BaseAdapter adapter)
	{
	...
	{
		inflater.inflate(
				message.direct() == EMMessage.Direct.RECEIVE ? R.layout.ease_row_received_video
	...
	protected void onSetUpView()
	{
		...
		if (localThumb != null)
		{
			...
			showVideoThumbView(localThumb, imageView,
		...
		EMLog.d(TAG,
				"video thumbnailStatus:" + videoBody.thumbnailDownloadStatus());
		if (message.direct() == EMMessage.Direct.RECEIVE)
		{
	 ...
	 * @param message
	 */
	private void showVideoThumbView(final String localThumb,
			final ImageView iv, String thumbnailUrl, final EMMessage message)
	{
		...
		} else
		{
			new AsyncTask<Void, Void, Bitmap>()
			{
				...
				protected Bitmap doInBackground(Void... params)
				{
					...
					{
						return ImageUtils
								.decodeScaleImage(localThumb, 160, 160);
					} else
					{
						return null;
					}
