...
public class BlacklistActivity extends Activity {
	...
	private class BlacklistAdapater extends ArrayAdapter<String> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			boolean isChinese = CommonUtils.isChinesewords(name.getText().toString());
//        boolean isNum = holder.nameView.getText().toString().matches("[0-9]+");
			String realname = DemoApplication.getInstance().mCache.getAsString(username + "_realname");
			Log.e("isNum", isChinese + "");
			if (!isChinese) {
				if (realname == null || realname.equals("")) {
					try {
						CommonUtils.getNickname(BlacklistActivity.this, username, name);
			...
			Glide.with(getContext())
					...
					.placeholder(com.wust.easeui.R.drawable.ease_default_avatar)
					.into(avatar);
